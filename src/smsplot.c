/*
SDB/SMSからthermal(0-25eV)のO+のデータを読み込み
gnuplotの(sms.plt)でカラーコンタが作れるようにファイル(smsploto.txt)に書き出す
...
/*ここからthermalデータ読み込み *1 */
			...
			/*ここまで*/
/*ここまで
*3部分を読みたい読み飛ばしている書き出したいイオン種の位置に移動するとそのイオン種の書き出しプログラムに
例）*2の前に移動すると、H+の書き出し*/

/*ここからsuprathermalデータの読み飛ばし*/
			for(k=0; k<=2; k++){
			for(i=0; i<=15; i++){
				dd = getc(file);
				df = ((float)dd)/50.0;
