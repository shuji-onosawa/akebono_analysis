...
pos_to_neg_idx, neg_to_pos_idx = find_zero_cross_idx(angle_b0_sBy_ary)
...
# calc Bpwr max and min for each channel
# max and min are used for ylim of plot
Bpwr_max_ary = sub_dataset['akb_mca_Bmax_pwr'].max(dim='Epoch').values
Bpwr_min_ary = sub_dataset['akb_mca_Bmax_pwr'].min(dim='Epoch').values

# plot angle_b0_sBy vs. Bpwr
for j in range(len(half_spin_idx_range_list)-2):
    if angle_b0_sBy_ary[half_spin_idx_range_list[j][0]] < 0:
        angle_b0_sBy_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]] = \
            angle_b0_sBy_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]] + 180
    if angle_b0_sBy_ary[half_spin_idx_range_list[j+1][0]] < 0:
        angle_b0_sBy_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]] = \
            angle_b0_sBy_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]] + 180
    if angle_b0_sBy_ary[half_spin_idx_range_list[j+2][0]] < 0:
        angle_b0_sBy_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]] = \
            angle_b0_sBy_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]] + 180

    # plot angle_b0_sBy vs. pwr for each channel
    for channel_idx in range(10):
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111)
        ax.plot(angle_b0_sBy_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]],
                Bpwr_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1], channel_idx], label='half spin 1')
        ax.plot(angle_b0_sBy_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]],
                Bpwr_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1], channel_idx], label='half spin 2')
        ax.plot(angle_b0_sBy_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]],
                Bpwr_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1], channel_idx], label='half spin 3')
        ax.set_title('Angle between B0 and sBy vs. Power for '+freq_label[channel_idx]+'\n' +
                        str(Epoch[half_spin_idx_range_list[j][0]])+' to '+str(Epoch[half_spin_idx_range_list[j+2][1]]))
        ax.set_xlabel('Angle between B0 and sBy (deg)')
        ax.set_ylabel('Power [pT^2/Hz]')
        ax.set_xlim(0, 180)
        ax.set_ylim(Bpwr_min_ary[channel_idx], Bpwr_max_ary[channel_idx])
        ax.legend()

        # save plot
        os.makedirs(save_dir+'/Bfiled/'+freq_label[channel_idx]+'/', exist_ok=True)
        plt.savefig(save_dir+'/Bfiled/'+freq_label[channel_idx]+'/spin'+str(j)+str(j+1)+str(j+2) + '.jpeg')


# find half spin index range for B field (Bloop)
pos_to_neg_idx, neg_to_pos_idx = find_zero_cross_idx(angle_b0_Bloop_ary)
half_spin_idx_range_list = []
...
# plot angle_b0_Bloop vs. Bpwr
for j in range(len(half_spin_idx_range_list)-2):
    if angle_b0_Bloop_ary[half_spin_idx_range_list[j][0]] < 0:
        angle_b0_Bloop_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]] = \
            angle_b0_Bloop_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]] + 180
    if angle_b0_Bloop_ary[half_spin_idx_range_list[j+1][0]] < 0:
        angle_b0_Bloop_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]] = \
            angle_b0_Bloop_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]] + 180
    if angle_b0_Bloop_ary[half_spin_idx_range_list[j+2][0]] < 0:
        angle_b0_Bloop_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]] = \
            angle_b0_Bloop_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]] + 180

    # plot angle_b0_Bloop vs. pwr for each channel
    for channel_idx in (10, 11):
        fig = plt.figure(figsize=(12, 8))
        ax = fig.add_subplot(111)
        ax.plot(angle_b0_Bloop_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1]],
                Bpwr_ary[half_spin_idx_range_list[j][0]:half_spin_idx_range_list[j][1], channel_idx], label='half spin 1')
        ax.plot(angle_b0_Bloop_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1]],
                Bpwr_ary[half_spin_idx_range_list[j+1][0]:half_spin_idx_range_list[j+1][1], channel_idx], label='half spin 2')
        ax.plot(angle_b0_Bloop_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1]],
                Bpwr_ary[half_spin_idx_range_list[j+2][0]:half_spin_idx_range_list[j+2][1], channel_idx], label='half spin 3')
        ax.set_title('Angle between B0 and Bloop vs. Power for '+freq_label[channel_idx]+'\n' +
                     str(Epoch[half_spin_idx_range_list[j][0]])+' to '+str(Epoch[half_spin_idx_range_list[j+2][1]]))
